...
export default class Home extends Component {
  ...
  //点击1级菜单
  goods1NameFn(rowID) {
    let data = this.state.LeftdataSource.splice(0);
    ...
    this.menuType = data[rowID].id;
    this.getGoodsList(data[rowID].id, rowID);
  }

  //一级菜单的list渲染
  _renderRow1(rowData, rowID) {
    return (
      <TouchableOpacity onPress={() => this.goods1NameFn(rowID)} style={[styles.goods1Name, rowData.active && styles.goods1NameActive]}>
  ...
  _renderRow2(listIndex, item, index) {
    ...
    return (
      <View style={styles.rowGoods} key={item.id}>
        ...
        <Text numberOfLines={1} style={styles.rowGoodsName}>{item.goodName}</Text>
...
const styles = StyleSheet.create({
  ...
  goods2: {
    width: '100%',
    height: scrrollHeight(pxToDp(314)),
    // paddingLeft: pxToDp(12),
    backgroundColor: "#f4f4f4",
  },
  goods2Header: {
